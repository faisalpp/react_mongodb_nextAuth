...
const Post = ({post, comments}) => {
  ...
  const validationSchema = Yup.object ({
   ...
   const handleSubmit = async (values) => {
    ...
    const data = await fetch('http://localhost:3000/api/addComments',{
      method:'POST',
      headers:{
  ...
  return (
   ...
   <ToastContainer position='top-left' autoClose={2000} hideProgressBar={false} newestOnTop={false} closeOnClick rtl={false} pauseOnFocusLoss draggable pauseOnHover/>
    <div className='flex flex-col rounded-xl ml-20 mt-20'>
   <div className='flex items-center mt-2  mb-2 border-2 border-gray-300 w-fit rounded-full px-2'><h3 className='text-c3'>Home</h3><IoIosArrowForward className='text-c2'/><h3 className='text-c3'>PostDetails</h3><IoIosArrowForward className='text-c2'/><h3 className='text-c3'>{post.title}</h3></div>
     <div className='flex items-center space-x-2 text-gray-500 mb-5'><Moment format="DD MMMM YYYY">{post.createdAt}</Moment><div className='bg-red-500 w-3 h-3 rounded-full'></div><h3>{post.category}</h3></div>
       <div className='font-bold text-7xl mb-5'>{post.title}</div>
       <div className='mt-10 mb-10 w-fit h-fit'><Image className='rounded-2xl' src={post.image} height={400} width={800}/></div>
       <div className='text-xl w-2/3 first-letter:text-5xl border-b-2 border-gray-300 py-10'>{content}</div>
    
    <div id="comments" className="w-96 mt-12">
